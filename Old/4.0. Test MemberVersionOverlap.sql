...
for select StartDate from #MemVer where HierId = @HierId and HierMemberId = @HierMemberId order by StartDate
open MemVer_cursor
...
while @@fetch_status =  0
	begin
		...
		while @@fetch_status =  0
			begin
				declare MapVer_cursor cursor --start MapVer for member and DstHier
				for select StartDate, EndDate, DstHierId, DstHierMemberId from #MapVer where SrcHierId = @HierId and SrcHierMemberId = @HierMemberId and DstHierId = @DstHierId order by StartDate
				open MapVer_cursor
				fetch next from MapVer_cursor
				into @MapStartDate, @MapEndDate, @DstHierId, @DstHierMemberId
				while @@fetch_status =  0
					begin
						if (@MemStartDate between @MapStartDate and @MapEndDate) or
							(@MapStartDate between @MemStartDate and @MemEndDate)
							begin
						
								declare DstMemVer_cursor cursor
								for select StartDate from #MemVer where HierId = @DstHierId and HierMemberId = @DstHierMemberId order by StartDate
								open DstMemVer_cursor
								fetch next from DstMemVer_cursor
								into @DstMemStartDate, @DstMemEndDate
								while @@fetch_status =  0
									begin
										if (@MemStartDate between @MapStartDate and @MapEndDate) and ()
											begin
												print 1
											end
									end
								close DstMemVer_cursor
								deallocate DstMemVer_cursor	
							end
					end
				close MapVer_cursor
				deallocate MapVer_cursor
				end
...
inner join
	#MemVer as MemVer
on
	Mem.HierId = MemVer.HierId and
	Mem.HierMemberId = MemVer.HierMemberId
where
	Mem.HierId = @HierId and
	Mem.HierMemberId = @HierMemberId
