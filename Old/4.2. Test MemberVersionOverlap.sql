...
drop table #MapEf
create table #MapEf (HierId tinyint, HierMemberId sys.hierarchyid, MemLifeDate date, DstHierId tinyint)
declare
	 @HierId tinyint = 1
	,@HierMemberId sys.hierarchyid = '/11/1/'
	,@MemStartDate date
	,@MemEndDate date
	,@DstHierId tinyint
	,@DstHierMemberId sys.hierarchyid
	...
	,@IterLevel tinyint
	,@col1 nvarchar(max)
	,@ParentHierMemberId sys.hierarchyid
		...
		while @@fetch_status =  0
			begin
				...
				while @MemLifeDate <= @MemEndDate
					begin
						insert into #MapEf values (@HierId, @HierMemberId, @MemLifeDate, @DstHierId)
...
set @IterLevel = @HierMemberId.GetLevel()
set @ParentHierMemberId = @HierMemberId
while @IterLevel > 0
	begin
		set @col1 = '[L' + cast(@IterLevel as varchar(max)) + '_DstHierMemberId]'
		set @sql = 'alter table #MapEf add ' + @col1 + ' sys.hierarchyid'
		exec sp_executesql @sql
		declare MapEf_cursor cursor --start in MapEf
		for select HierId, HierMemberId, MemLifeDate, DstHierId from #MapEf
		open MapEf_cursor
		fetch next from MapEf_cursor
		into @HierId, @HierMemberId, @MemLifeDate, @DstHierId
		while @@fetch_status =  0
			begin
				set @DstHierMemberId = (select DstHierMemberId from #MapVer where SrcHierId = @HierId and SrcHierMemberId = @ParentHierMemberId and DstHierId = @DstHierId and @MemLifeDate between StartDate and EndDate)
				if len(@DstHierMemberId.ToString()) > 0
					begin
						if (select count(1) from #MemVer where HierId = @DstHierId and HierMemberId = @DstHierMemberId and @MemLifeDate between StartDate and EndDate) = 1
							begin
								set @sql = 'update #MapEf set ' + @col1 + ' = ''' + @DstHierMemberId.ToString() + ''' where HierId = ' + cast(@HierId as nvarchar(max)) + ' and HierMemberId = ''' + @HierMemberId.ToString() + ''' and MemLifeDate = ''' + cast(@MemLifeDate as nvarchar(max)) + ''' and DstHierId = ' + cast(@DstHierId as nvarchar(max))
								print @sql
								exec sp_executesql @sql
							end
					end
		...
		set @IterLevel = @IterLevel - 1
		set @ParentHierMemberId = @ParentHierMemberId.GetAncestor(1)
	end
...
select
	 HierId
	,HierMemberId.ToString()
	,MemLifeDate
	,DstHierId
--	,L3_DstHierMemberId.ToString()
	,L2_DstHierMemberId.ToString()
	,L1_DstHierMemberId.ToString()
from #MapEf
where not(
--	[L3_DstHierMemberId] is null and
	[L2_DstHierMemberId] is null and
	[L1_DstHierMemberId] is null
)
